...
def build() -> str:
    ...
    lines = [
        ...
        ("9-副本难度   桌面副本的 10 档难度配置", False, 10),
        ("", False, 10),
        ("注意事项：", True, 11),
    ...
    for lv in range(1, 81):
        ...
        if lv <= 10:
            ...
            # 高等级加大 HP、放缓攻/速成长 —— 保证战斗时长不随等级缩水
            hp, atk, arm, spd, sta = 30, 2, 2, 0, 2
    ...
    write_sheet(
        wb,
        "9-副本难度",
        ["ID", "名称", "每层敌人数", "等级下限", "等级上限", "品种池",
         "HP倍率", "攻击倍率", "护甲倍率", "速度倍率", "耐力倍率",
         "单杀经验", "层通关经验", "解锁前置"],
        [
            ["d01", "新手", 3, 1, 6, "全部", 0.85, 0.80, 0.85, 0.95, 0.95, 11, 33, ""],
            ["d02", "入门", 3, 6, 13, "全部", 0.90, 0.85, 0.90, 1.00, 1.00, 20, 60, "d01"],
            ["d03", "普通", 3, 11, 18, "全部", 0.95, 0.90, 0.95, 1.05, 1.05, 26, 78, "d02"],
            ["d04", "初级", 4, 16, 24, "全部", 0.95, 0.95, 0.95, 1.05, 1.05, 37, 111, "d03"],
            ["d05", "中级", 4, 22, 30, "全部", 1.00, 1.00, 1.00, 1.05, 1.05, 45, 135, "d04"],
            ["d06", "高级", 5, 28, 37, "全部", 1.10, 1.05, 1.05, 1.10, 1.10, 55, 165, "d05"],
            ["d07", "噩梦", 5, 35, 44, "全部", 1.20, 1.10, 1.10, 1.10, 1.10, 77, 231, "d06"],
            ["d08", "深渊", 5, 43, 52, "全部", 1.20, 1.10, 1.10, 1.10, 1.10, 105, 315, "d07"],
            ["d09", "绝望", 6, 52, 62, "全部", 1.05, 1.00, 1.00, 1.10, 1.10, 116, 348, "d08"],
            ["d10", "真神", 7, 62, 72, "全部", 1.05, 1.05, 1.05, 1.15, 1.15, 142, 426, "d09"],
        ],
        [8, 10, 12, 10, 10, 10, 9, 10, 10, 10, 10, 11, 12, 11],
        note="每层敌人数：该难度每一层要连续击败的敌人数量；等级在上下限之间随层数线性提升；"
             "品种池填品种ID逗号分隔，「全部」表示所有品种；倍率乘在怪物最终属性上；"
             "单杀经验随层数每层+5%；第 20 层为首领层（额外 1.35 倍强度）；解锁前置为需先通关的难度ID",
        highlight={0: GOOD_FILL, 9: WARN_FILL},
    )
