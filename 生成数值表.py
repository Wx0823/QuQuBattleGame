...
def build() -> str:
    ...
    lines = [
        ...
        ("10-装备部位   装备槽位定义（可扩展）", False, 10),
        ("11-装备品质   品质/词条数/数值倍率（可扩展）", False, 10),
        ("12-装备词条   部位×属性的加成词条池（可扩展）", False, 10),
        ("", False, 10),
        ("注意事项：", True, 11),
    ...
    write_sheet(
        ...
        ["ID", "名称", "每层敌人数", "等级下限", "等级上限", "品种池",
         ...
         "单杀经验", "层通关经验", "解锁前置", "装备掉落概率", "装备品质权重"],
        [
            ["d01", "新手", 3, 1, 6, "全部", 0.85, 0.80, 0.85, 0.95, 0.95, 11, 33, "", 20, "q01:90,q02:9,q03:1"],
            ["d02", "入门", 3, 6, 13, "全部", 0.90, 0.85, 0.90, 1.00, 1.00, 20, 60, "d01", 24, "q01:82,q02:14,q03:4"],
            ["d03", "普通", 3, 11, 18, "全部", 0.95, 0.90, 0.95, 1.05, 1.05, 26, 78, "d02", 28, "q01:74,q02:18,q03:7,q04:1"],
            ["d04", "初级", 4, 16, 24, "全部", 0.95, 0.95, 0.95, 1.05, 1.05, 37, 111, "d03", 32, "q01:65,q02:21,q03:11,q04:3"],
            ["d05", "中级", 4, 22, 30, "全部", 1.00, 1.00, 1.00, 1.05, 1.05, 45, 135, "d04", 36, "q01:55,q02:24,q03:15,q04:5,q05:1"],
            ["d06", "高级", 5, 28, 37, "全部", 1.10, 1.05, 1.05, 1.10, 1.10, 55, 165, "d05", 40, "q01:45,q02:25,q03:18,q04:8,q05:3,q06:1"],
            ["d07", "噩梦", 5, 35, 44, "全部", 1.20, 1.10, 1.10, 1.10, 1.10, 77, 231, "d06", 44, "q01:35,q02:24,q03:21,q04:12,q05:5,q06:2.5,q07:0.5"],
            ["d08", "深渊", 5, 43, 52, "全部", 1.20, 1.10, 1.10, 1.10, 1.10, 105, 315, "d07", 48, "q01:25,q02:22,q03:23,q04:15,q05:9,q06:4.5,q07:1.5"],
            ["d09", "绝望", 6, 52, 62, "全部", 1.05, 1.00, 1.00, 1.10, 1.10, 116, 348, "d08", 52, "q01:15,q02:18,q03:24,q04:19,q05:13,q06:8,q07:3"],
            ["d10", "真神", 7, 62, 72, "全部", 1.05, 1.05, 1.05, 1.15, 1.15, 142, 426, "d09", 56, "q01:8,q02:14,q03:22,q04:22,q05:18,q06:11,q07:5"],
        ],
        [8, 10, 12, 10, 10, 10, 9, 10, 10, 10, 10, 11, 12, 11, 13, 40],
        note="每层敌人数：该难度每一层要连续击败的敌人数量；等级在上下限之间随层数线性提升；"
             ...
             "装备掉落概率为击败每个怪物的掉落几率(%)；装备品质权重格式 qID:权重 逗号分隔",
    ...
    write_sheet(
        ...
        ["ID", "名称", "词条倾向", "描述"],
        [
            ["p01", "触须", "速度、耐力回复", "感知灵敏，久战不疲"],
            ["p02", "牙齿", "攻击、暴击", "牙口决定输出"],
            ["p03", "前躯", "血量、护甲", "躯干护持，能抗能打"],
            ["p04", "后躯", "耐力、斗性", "腰腹爆发，斗志顽强"],
            ["p05", "翅膀", "速度、韧性", "振翅生风，更难被暴击"],
            ["p06", "尾巴", "破甲、血量", "平衡尾舵，撕开防御"],
        ],
        [8, 10, 22, 36],
        note="部位可动态扩展：加行即生效。词条倾向仅作设计说明，实际加成由「12-装备词条」的部位列决定",
    )
    ...
    write_sheet(
        wb,
        "11-装备品质",
        ["ID", "名称", "颜色", "词条数", "数值倍率", "描述"],
        [
            ["q01", "白色", "#D5D8DC", 1, 1.00, "普通装备"],
            ["q02", "夫勇", "#7DBB6C", 1, 1.30, "悍勇装备（绿）"],
            ["q03", "蓝色", "#4A90D9", 2, 1.65, "铁甲装备"],
            ["q04", "紫色", "#A06BDC", 2, 2.05, "纯银装备"],
            ["q05", "金色", "#E8B23A", 3, 2.55, "炫金装备"],
            ["q06", "橙色", "#E87A2C", 3, 3.20, "无畏装备"],
            ["q07", "黑金", "#3A3A45", 4, 4.00, "先锋装备（词条按金色列）"],
        ],
        [8, 10, 10, 9, 10, 30],
        note="品质可动态扩展：加行即生效。数值倍率乘在词条基础值上；颜色为 UI 显示色",
        highlight={6: GOOD_FILL},
    )
    ...
    write_sheet(
        wb,
        "12-装备词条",
        ["ID", "名称", "部位", "属性ID", "基础值", "权重", "描述"],
        [
            ["e01", "利齿", "牙齿", "atk", 12, 100, "咬合更有力，攻击提升"],
            ["e02", "锋颚", "牙齿", "atk", 18, 40, "同为大攻词条，数值更高更稀有"],
            ["e03", "獠牙", "牙齿", "pen", 8, 55, "撕裂防御，破甲提升"],
            ["e04", "迅捷触须", "触须", "spd", 6, 100, "反应更快，速度提升"],
            ["e05", "灵敏感须", "触须", "spd", 10, 40, "高敏词条，速度大幅提升"],
            ["e06", "感知须", "触须", "sta_regen", 3, 80, "感知环境，耐力回复提升"],
            ["e07", "坚壳前躯", "前躯", "arm", 10, 100, "外壳硬化，护甲提升"],
            ["e08", "厚甲前胸", "前躯", "hp", 30, 80, "胸甲厚实，血量提升"],
            ["e09", "蛮力前肢", "前躯", "atk", 8, 60, "前肢有力，攻击提升"],
            ["e10", "爆发后躯", "后躯", "sta", 20, 100, "蹬地有力，耐力提升"],
            ["e11", "铁腰", "后躯", "guts", 10, 80, "腰杆硬，斗志更顽强"],
            ["e12", "强健后腿", "后躯", "hp", 25, 70, "后腿粗壮，血量提升"],
            ["e13", "轻盈翅膜", "翅膀", "spd", 8, 90, "振翅迅捷，速度提升"],
            ["e14", "韧翅", "翅膀", "tough", 5, 80, "翅面坚韧，降低被暴击"],
            ["e15", "鸣音翅", "翅膀", "guts", 8, 50, "鸣声洪亮，斗志提升"],
            ["e16", "平衡尾", "尾巴", "pen", 10, 90, "尾舵稳定发力，破甲提升"],
            ["e17", "稳尾", "尾巴", "hp", 22, 80, "尾身扎实，血量提升"],
            ["e18", "力尾", "尾巴", "atk", 7, 50, "甩尾发力，攻击提升"],
        ],
        [8, 12, 10, 10, 9, 8, 34],
        note="部位可填多个（逗号分隔）或「全部」；属性ID须在「1-属性定义」中登记；"
             "最终数值 = 基础值 × 品质数值倍率 × 随机浮动(0.85~1.25)，并有 3% 概率按高 1 档品质生成",
        highlight={1: NOTE_FILL},
    )
